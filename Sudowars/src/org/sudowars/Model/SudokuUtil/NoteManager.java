...
public class NoteManager implements Serializable {
	 ...
	 */
	public GameCell getSelectedCell(SudokuField sudokuField) {
		if (sudokuField.selectedFieldX != -1 && sudokuField.selectedFieldY != -1)
			return sudokuField.game.getSudoku().getField().getCell(sudokuField.selectedFieldX, sudokuField.selectedFieldY);
	 ...
	 * @param sudokuField TODO
	 */
	public void setNoteManager(SudokuField sudokuField) {
		sudokuField.noteManager = this;
	}
	 ...
	 */
	public void notificate(Play play, int resId, int duration) {
		long now = SystemClock.uptimeMillis();
		
		if (play.preferences.getBoolean("notifications", true) && now - play.lastNotificationTime > play.getResources().getInteger(integer.notification_interval)) {
			Toast.makeText(play.getApplicationContext(), resId, duration).show();
		}
		
		play.lastNotificationTime = now;
	}
