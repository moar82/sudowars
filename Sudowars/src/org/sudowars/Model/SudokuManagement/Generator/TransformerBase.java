 ...
 */
public class TransformerBase implements Serializable {
	...
	public Sudoku<DataCell> pickSudokuRandomly(SudokuFilePool sudokuFilePool, File[] foundSudokus) {
		assert foundSudokus != null && foundSudokus.length > 0;
		//pick sudoku randomly
		int index = sudokuFilePool.randomNumberGenerator.nextInt(foundSudokus.length);
		Sudoku<DataCell> result;
		try {
			result = SudokuFilePool.loadSudoku(foundSudokus[index]);
			foundSudokus[index].delete();
			sudokuFilePool.hasChanged = true;
		} catch (IOException e) {
			result = null;
		}
		return result;
