...
public class SudokuFilePool extends Service implements SudokuPool {
	...
	public final Random randomNumberGenerator;
	...
	public SudokuFilePool() {
		this.transformer = new TransformerBase();
	...
	private void startSudokuGeneration(Sudoku<DataCell> baseSudoku, Difficulty difficulty) {
		...
		GeneratorBase generator = new GeneratorBase(this);
	...
	public Sudoku<DataCell> extractSudoku(FieldStructure structure, Difficulty difficulty) throws IllegalArgumentException{
		...
		File sudokuFile = difficulty.getDirectoryForSudokuType(structure, this);
		if (sudokuFile != null) {
		...
		if (foundSudokus.length > 0) {
			Sudoku<DataCell> result = transformer.pickSudokuRandomly(this, foundSudokus);
			if (result != null) {
	...
	public
	static Sudoku<DataCell> loadSudoku(File sudokuFile) throws IOException {
	...
	private File generateSudokuFile(FieldStructure structure, Difficulty difficulty) {
		...
		File baseDirectory = difficulty.getDirectoryForSudokuType(structure, this);
	...
	public synchronized boolean addSudoku(Sudoku<DataCell> sudoku, Difficulty difficulty, boolean transform) throws IllegalArgumentException {
		...
		if (difficulty.getSudokuCount(sudoku.getField().getStructure(), this) < MAX_SUDOKU_COUNT) {
			try {
