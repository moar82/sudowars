...
public class GameCell implements Cell {
	...
	private static boolean attributesEqual(GameCell first, GameCell second) {
		...
		&& PlayerSlot.objectsEqual(first.playerSlot, second.playerSlot));
	}
	...
	public Paint getPaintForCell(SudokuField sudokuField, int x, int y, boolean activeCellInvalid) {
		Player localPlayer = sudokuField.game.getPlayers().get(0);
		
		if (isInitial())
			return sudokuField.initialPaint;
		if (sudokuField.game instanceof MultiplayerGame) {
			if (isOwnerPending())
				return sudokuField.pendingPaint;
			if (getOwningPlayer() == null)
				return sudokuField.pendingPaint;
			if (getOwningPlayer().equals(localPlayer)) {
				if (sudokuField.selectedFieldX == x && sudokuField.selectedFieldY == y) 
					return sudokuField.normalPaintSelected;
				else
					return sudokuField.normalPaint;
			}else {
				if (sudokuField.markedCells[x][y] != 0)
					return sudokuField.normalPaintSelected;
				
				return sudokuField.opponentPaint;
			}
		}else {
			if (sudokuField.selectedFieldX == x && sudokuField.selectedFieldY == y) {
				if (activeCellInvalid)
					return sudokuField.normalPaintInvalid;
				return sudokuField.normalPaintSelected;
			}else {
				return sudokuField.normalPaint;
			}	
	 ...
	 */
	public boolean isEditable(Play play) {
		boolean error = false;
		
		if (this == null) {
			play.noteManager.notificate(play, string.notification_unselected_field, Toast.LENGTH_SHORT);
			error = true;
		}
	
		if (!error && isInitial()) {
			play.noteManager.notificate(play, string.notification_initial_field, Toast.LENGTH_SHORT);
			error = true;
		}
		
		if (!error && play.assistantRunning) {
			play.noteManager.notificate(play, string.notification_assistant_running, Toast.LENGTH_SHORT);
			error = true;
		}
