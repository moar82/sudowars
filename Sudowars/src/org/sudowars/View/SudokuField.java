...
public class SudokuField extends View {
	 ...
	 */
	public Paint normalPaintInvalid;
	
	/**
	 * Paint to draw the bold lines 
	 */
	public Paint boldLinePaint;
	 ...
	 */
	public Paint normalPaintSelected;
	
	/**
	 * Normal paint for initial fields
	 */
	public Paint initialPaint;
	
	/**
	 * Paint for the symbols of the opponent
	 */
	public Paint opponentPaint;
	 ...
	 */
	public Paint pendingPaint;
	
	/**
	 * actual selected field X
	 */
	public int selectedFieldX = -1;
	
	/**
	 * actual selected field Y
	 */
	public int selectedFieldY = -1;
	 ...
	 * The scale factor
	 */
	public float scaleFactor = (float) 1.0;
	...
	protected void onDraw(Canvas canvas) {
		...
		if (this.game == null) {
			this.game.drawDummy(this, canvas);
			return;
		}
      ...
      //This is not a solution for all Blocks Problem, tough we only support 9x9 and 16x16 Sudokus, this one is valid
		...
		for (int x = 0; x < this.game.getSudoku().getField().getStructure().getWidth(); x++) {
			for (int y = 0; y < this.game.getSudoku().getField().getStructure().getHeight(); y++) {
				...
				if (gc.getValue() != 0) {
					tmpPaint = gc.getPaintForCell(this, x, y, activeCellInvalid);
	...
	public void setGame (Game game) {
		if (game != null) {
			...
			this.game.getPlayers().get(0).getNoteManagerOfPlayer(this.game).addOnChangeListener(new NoteManagerChangedEventListener() {
	 ...
	 */
	public void vibrate(Play play, long milliseconds) {
		if (play.vibrator != null) {
			play.vibrator.vibrate(milliseconds);
		}
	}
}
