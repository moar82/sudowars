...
public class SingleplayerPlay extends Play {
	...
	protected void onResume() {
		...
		if (!this.gameState.isFinished()) {
			this.localPlayer.resumeGame(this.game);
			this.game.startGame();
		}
	}
	...
	protected void onPause() {
		...
		this.localPlayer.pauseGame(this.game);
	}
		...
		if (item.getItemId() == R.id.assistant) {
			...
			if (((SingleplayerGameState) this.gameState).isBackToFirstErrorEnabled()
				...
				if (this.deltaManager.backToFirstError((SingleplayerGame) this.game, this.localPlayer)) {
					...
					noteManager.notificate(this, R.string.notification_back_to_first_error_success, Toast.LENGTH_LONG);
					return true;
				} else {
					//there was no error
					noteManager.notificate(this, R.string.notification_back_to_first_error_failed, Toast.LENGTH_LONG);
					return false;
				}
			}
			return true;
	...
	protected boolean onSymbolToggled(int symbolId) {
		...
		if (!error) {
			...
			if (selectedCell.isSet()) {
				...
				((CompositeCommand) command).pushCommand(new AddNoteCommand(selectedCell, this.sudokuField.noteManager.getSelectedCell(this.sudokuField).getValue()));
	...
	protected boolean onSymbolLongPress(int symbolId) {
		...
		if (!error) {
			...
			if (!selectedCell.isSet() && localPlayer.getNoteManagerOfPlayer(this.game).getNotes(selectedCell).isEmpty()) {
				command = new CompositeCommand();
				((CompositeCommand) command).pushCommand(new ClearCellCommand(selectedCell));
	...
	private boolean handleObjectItemAssistant() throws IllegalArgumentException {
		if (((SingleplayerGameState) this.gameState).isSolveCellEnabled() && !this.game.isPaused()
			...
			if (!assistantRunning) {
				...
				noteManager.notificate(this, R.string.notification_assistant_running, Toast.LENGTH_SHORT);
			}
