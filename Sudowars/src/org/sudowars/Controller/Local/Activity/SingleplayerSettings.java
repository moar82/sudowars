...
import org.sudowars.Model.SudokuUtil.SingleplayerGameStateParameter;
...
public class SingleplayerSettings extends Settings {
	...
	private void onBtnStartClick () {
		...
		player.setNoteManagerOfPlayer(game, noteManager);
		
		FileIO savedGames = new FileIO(this.getApplicationContext());
		//call of the ro candidate 186
		
		
		savedGames.saveSingleplayerGame(new SingleplayerGameState(new SingleplayerGameStateParameter(game, difficulty, this.preferences.getBoolean("singleplayer_assistant_show_mistakes", true),
				this.preferences.getBoolean("singleplayer_assistant_solve_cells", true), this.preferences.getBoolean("singleplayer_assistant_bookmark", true), this.preferences.getBoolean("singleplayer_assistant_back_to_first_error", true), sudoku.getField(), new DeltaManager())));
		//Debug output
